...
export function createInterpreterKernelSpec(
    ...
    const defaultSpec: Kernel.ISpecModel = {
        name: rootKernelFilePath ? path.dirname(rootKernelFilePath) : getInterpreterKernelSpecName(interpreter),
        language: 'python',
...
export function findPreferredKernelIndex(
    ...
    if (index < 0) {
        ...
        for (let i = 0; kernels && i < kernels?.length; i = i + 1) {
            ...
            if (spec) {
                // Check if the hash names match the interpreter & kernel spec name (that we generated).
                if (
